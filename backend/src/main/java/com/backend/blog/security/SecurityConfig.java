...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and()       //CorsFilter 라는 필터가 존재하는데 이를 활성화 시키는 작업
                ...
                //.httpBasic().disable()// 일반적인 루트가 아닌 다른 방식으로 요청시 거절, header에 id, pw가 아닌 token(jwt)을 달고 간다. 그래서 basic이 아닌 bearer를 사용한다.
                .sessionManagement()
                ...
                //.antMatchers(HttpMethod.OPTIONS, "/**").permitAll() //antMatchers = 스프링 시큐리티에서 인증이 되지 않더라도 통과시켜 누구에게나 사용을 열어줍니다.
                .antMatchers("/user/login").permitAll() //permitAll = 스프링 시큐리티에서 인증되지 않더라고 통과시켜 누구에게나 사용을 열어줌
                //.antMatchers("/api/user").authenticated() // 요청내에 스프링 시큐리티 컨텍스트 내에서 인증이 완료되어야 api를 사용할 수 있습니다. 인증이 되지 않은 요청은 403(Forbidden)이 내려집니다.
